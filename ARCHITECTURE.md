...
## 13. Remote workers on an external Kubernetes cluster

Compute-heavy pipelines (MRIQC/FreeSurfer/LST-AI) can be offloaded to a Kubernetes cluster
hosted at a different site, while **management stays strictly internal** — only compute
runs remotely. Full detail: `deploy/k8s-remote-worker/README.md`.

- **Connectivity**: a point-to-point WireGuard tunnel between two gateway containers — one in
  `deploy/full/docker-compose.yml` (service `wireguard-gateway`), one in the remote cluster
  (`deploy/k8s-remote-worker/wireguard-gateway.yaml`) — rather than routing whole subnets. Each
  gateway forwards only the two ports that matter (5671 AMQPS, 443 HTTPS) into its peer's tunnel
  IP via `socat`; no general L3 routing into either side's internal network. The remote cluster
  (fixed IP) is the WireGuard "server" side (listens on a `NodePort` UDP `Service`); Girder's
  side dials out, so no inbound port needs to be opened on Girder's own network. Remote worker
  pods reach RabbitMQ/Girder only via the in-namespace `Service wireguard-gateway`, never a raw
  IP — see `deploy/k8s-remote-worker/networkpolicy.yaml` for the egress allow-list (restricted
  to that one pod).
- **Broker channel**: RabbitMQ gets a TLS listener (AMQPS, port 5671, `deploy/full/rabbitmq/rabbitmq.conf`)
  in addition to the existing plaintext 5672 used by local workers. A dedicated RabbitMQ user
  (`init-rabbitmq-remote` service in `deploy/full/docker-compose.yml`) is provisioned on the
  **same default vhost** as local workers — a separate vhost would isolate the remote worker
  from the existing queues and require changing the task-dispatch side too — but with
  permissions restricted by regex to only the queue(s) offloaded remotely
  (`REMOTE_WORKER_QUEUES_REGEX`, default `^diadema_mriqc$`). A compromised remote cluster
  therefore cannot see or touch the other pipelines' queues, exchanges, or the management API.
- **No MongoDB access from remote workers**: MongoDB is Girder's own database *and* the Celery
  result backend (`CELERY_RESULT_BACKEND` in the `x-celery-env` compose anchor) — remote worker
  manifests set `CELERY_RESULT_BACKEND=rpc://` instead (RabbitMQ-based), so MongoDB never needs
  to be reachable from the remote site. In practice DIADEMA tasks already set
  `task_ignore_result = True` (`worker_entry.py`), so no task result is ever written there.
- **Job routing requires no application code change**: which pipeline runs remotely is decided
  purely by which queue name (`--queues=...`) the remote worker container consumes — the same
  mechanism already used to dedicate one worker container per pipeline locally. Offloading
  FreeSurfer/LST-AI follows the same pattern as the MRIQC example in `deploy/k8s-remote-worker/`.
- **Data path unchanged**: remote workers never mount the assetstore. Input/output still flow
  exclusively through the Girder REST API over HTTPS (§8), the one channel already prepared for
  cross-network exposure (internal CA + `REQUESTS_CA_BUNDLE`).
- **Scheduling on the remote cluster**: workers are deployed as a KEDA `ScaledJob`
  (`deploy/k8s-remote-worker/scaledjob-mriqc.yaml`), not a permanent `Deployment` — one
  Kubernetes `Job` is created per queued message (RabbitMQ scaler), scaling to zero when idle.
  A low `PriorityClass` (`priorityclass.yaml`) keeps these batch jobs preemptible if the remote
  cluster is shared with other workloads.
- **Container images**: built locally (`deploy/full/docker-compose.yml`, `build.context: ../../`).
  The MRIQC worker image is published to GitHub Container Registry
  (`ghcr.io/marver17/diadema-mriqc-worker`, private package) via a manual `docker build`/`docker push`
  — there is still no CI automation to rebuild/republish it on every commit, see the prerequisites
  in `deploy/k8s-remote-worker/README.md`.
- **TLS hostname for the tunnel**: the server certificate generated by `init-certs` needs the
  Kubernetes `Service` DNS name (`wireguard-gateway.diadema-remote.svc.cluster.local`) as an
  extra SAN, since remote workers validate TLS against that hostname, not `nginx`/`localhost`.
  Set via `DIADEMA_EXTRA_SAN` in `deploy/full/.env` before `init-certs` first generates the
  certificate.
