...
from .widget_registry import get_widget_providers, register_widget_provider
from .widget_rest import WidgetResource
...
class NiftiViewerPlugin(GirderPlugin):
    ...
    def load(self, info):
        ...
        # Register widget data REST endpoint
        widgetResource = WidgetResource()
        info["apiRoot"].item.route(
            "GET", (":id", "nifti_widgets"), widgetResource.getWidgetData
        )
        ...
        # Store registry function in info for other plugins to access directly
        # This allows plugins loading after us to register immediately
        info['nifti_widget_registry'] = {
            'register_function': register_widget_provider,
            'get_providers': get_widget_providers
        }
        
        print(" NIfTI Viewer Plugin loaded successfully!")
        print("   - Widget registry: ready")
        
        # Trigger event for plugins that already did events.bind()
        # This handles plugins that loaded before us
        events.trigger(
            "nifti_viewer.register_widgets",
            {"register_function": register_widget_provider},
        )
...
class NiftiItem(Resource):
    ...
    @autoDescribeRoute(
        Description(
            "Parse NIfTI files and extract metadata from NIfTI header and optional JSON sidecar"
        )
        .modelParam(
            "id", "The item ID", model="item", level=AccessType.WRITE, paramType="path"
        )
    ...
    def makeNiftiItem(self, item):
        ...
        # Store in the item
        item["nifti"] = {"meta": combinedMeta, "files": files}
...
def _get_space_units(header):
    ...
    units_dict = {0: "unknown", 1: "meter", 2: "mm", 3: "micron"}
    try:
...
def niftiSubstringSearchHandler(query, types, user=None, level=None, limit=0, offset=0):
    ...
    search_query = {"nifti": {"$exists": True}, "$or": conditions}

    logger.info(f"MongoDB query: {search_query}")

    # Esegui ricerca
    items = Item().findWithPermissions(
        search_query, user=user, level=level, limit=limit, offset=offset
    )
    ...
    for item in items:
        ...
        if len(filtered_items) <= 3:
            logger.info(
                f'Result {len(filtered_items)-1}: {item.get("name", "N/A")} (ID: {item["_id"]})'
            )
            if "nifti" in item and "meta" in item["nifti"]:
                logger.info(
                    f'  Orientation: {item["nifti"]["meta"].get("orientation", "N/A")}'
                )
